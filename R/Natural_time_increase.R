...
#'
#' @import dplyr
#' @return a text informing of the fastest duration for increase
#' @export
#'
#' @examples Natural_time_increase <- function(Stream_Module=10, Start=50,End=150, b_drop = 0.75, b_rise = 1.15)
Natural_time_increase <- function(Stream_Module, Start=50,End=150, b_drop = 0.75, b_rise = 1.15) {
  ...
  table <- table %>%
    ...
    filter(Percent_of_Stream_Module==End|Percent_of_Stream_Module==Start) %>%
    pull(RiseTime_h)
  Duration<-table[1]-table[2]
  Duration<-paste0(round(Duration,digits = 1)," hours")
  return(Duration)
}
